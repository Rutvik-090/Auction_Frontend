...
const AuctionDetails = () => {
  ...
  useEffect(() => {
    const fetchAuctionData = async () => {
      try {
        const { data: auctionData } = await axios.get(`http://localhost:5000/api/auctions/${id}`);
        setAuction(auctionData);

        const { data: bidsData } = await axios.get(`http://localhost:5000/api/bids/${id}`);
        setBidHistory(bidsData);

        if (auctionData.category) {
           const { data: allAuctions } = await axios.get(`http://localhost:5000/api/auctions`);
           const similar = allAuctions.filter(a => a.category === auctionData.category && a._id !== id).slice(0, 3);
    ...
    socket.on('new_bid', (data) => {
      if (data.auctionId === id) {
        ...
        setBidHistory((prev) => [
          { bidder: { name: data.bidder }, amount: data.amount, time: new Date() },
          ...prev
        ]);
      }
    });
      ...
      <div className="max-w-[1440px] mx-auto px-8 py-12">
          ...
          <div className="lg:col-span-5 flex flex-col justify-center">
            <section className="space-y-4 mb-8">
              <nav className="flex items-center gap-2 text-on-surface-variant text-xs uppercase font-bold tracking-widest mb-4 opacity-70">
                <Link to="/browse" className="hover:text-primary transition-colors">Gallery</Link>
            ...
            <div className="bg-surface-container-lowest p-8 rounded-3xl shadow-[0_32px_64px_-16px_rgba(53,37,205,0.08)] border border-outline-variant/20 space-y-8 relative overflow-hidden">
              ...
              {auction.status === 'active' && user ? (
                <form onSubmit={handleBid} className="space-y-4 relative z-10 block">
                  ...
                  <button type="submit" className="w-full bg-gradient-to-r from-primary to-primary-container text-white py-5 rounded-full font-headline font-black tracking-wide text-xl shadow-lg shadow-primary/20 hover:shadow-primary/40 active:scale-95 transition-all outline-none">
                    Place Bid
                  </button>
                  <p className="text-[10px] text-center text-on-surface-variant uppercase tracking-widest opacity-60 font-bold mt-2">By bidding, you agree to our terms</p>
                </form>
              ) : !user ? (
                 <div className="mt-6 text-center p-6 bg-surface-container-high rounded-xl text-on-surface-variant font-bold tracking-wide border border-outline-variant/20 shadow-inner">
                  Please <Link to="/login" className="text-primary hover:underline">Log in</Link> to place a bid.
                </div>
              ) : auction.status === 'ended' && auction.highestBidder?.name === user.name ? (
                <div className="mt-6">
                   <button className="w-full bg-gradient-to-r from-green-500 to-green-600 text-white font-headline font-black text-lg py-5 rounded-full shadow-lg hover:shadow-xl active:scale-95 transition-all" onClick={() => alert('Payment gateway pending')}>
              ...
              {error && <div className="mt-4 text-sm font-bold text-error bg-error-container/50 p-4 rounded-xl flex items-center gap-2"><span className="material-symbols-outlined text-sm">error</span>{error}</div>}
              {success && <div className="mt-4 text-sm font-bold text-secondary bg-secondary-container/50 p-4 rounded-xl flex items-center gap-2"><span className="material-symbols-outlined text-sm">check_circle</span>{success}</div>}
            </div>
            ...
            <div className="flex items-center gap-4 mt-8 px-4">
               ...
               <div>
                 <div className="flex items-center gap-1">
                   <span className="font-bold text-sm text-on-surface">{auction.seller?.name || 'Unknown Curator'}</span>
                   <span className="material-symbols-outlined text-secondary text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>verified</span>
                 </div>
                 <div className="text-[10px] uppercase font-bold tracking-widest text-outline">Premium Seller</div>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 pt-16 border-t border-outline-variant/20">
            ...
            <div className="lg:col-span-8">
              <h3 className="font-headline font-black text-2xl tracking-tighter mb-8 text-on-surface">About the Item</h3>
            ...
            <div className="lg:col-span-4">
               <h3 className="font-headline font-black text-2xl tracking-tighter mb-8 text-on-surface flex items-center gap-3">
                 <span className="material-symbols-outlined text-primary">history</span> Bid History
               </h3>
               ...
               <div className="bg-surface-container-lowest rounded-3xl shadow-sm border border-outline-variant/20 overflow-hidden">
                 {bidHistory.length > 0 ? (
                   ...
                   <div className="p-8 text-center text-outline">
                      <span className="material-symbols-outlined text-4xl opacity-50 mb-2">gavel</span>
                      <p className="font-bold text-sm">No bids placed yet.</p>
                      <p className="text-xs mt-1">Be the first to bid on this exclusive item.</p>
                   </div>
                 )}
               </div>
            </div>
        ...
        {similarProducts.length > 0 && (
          <div className="pt-24 mt-16 border-t border-outline-variant/20">
             <div className="flex items-center justify-between mb-10">
               <div>
                  <h3 className="text-primary font-bold text-[10px] tracking-widest uppercase mb-2">Curated Matches</h3>
                  <h2 className="font-headline font-black text-3xl tracking-tighter text-on-surface">Similar Masterpieces</h2>
               </div>
               <Link to="/browse" className="hidden md:flex items-center gap-1 font-bold text-sm text-primary hover:text-primary-container transition-colors">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               {similarProducts.map((item) => (
                  <Link key={item._id} to={`/auction/${item._id}`} className="group bg-surface-container-lowest rounded-2xl overflow-hidden shadow-sm border border-outline-variant/20 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl block">
                    <div className="relative h-48 overflow-hidden rounded-t-xl bg-surface-container shrink-0">
                      ...
                      <div className={`absolute top-3 left-3 bg-surface-container-lowest/90 backdrop-blur-md px-3 py-1.5 rounded-full flex items-center gap-2 ${item.status !== 'active' ? 'opacity-70 grayscale' : ''}`}>
                        <span className={`w-2 h-2 rounded-full ${item.status === 'active' ? 'bg-secondary animate-pulse' : 'bg-outline-variant'}`}></span>
                        <span className="text-[10px] font-black uppercase tracking-widest font-label">{item.status === 'active' ? 'Live' : 'Ended'}</span>
                      </div>
                    </div>
                    
                    <div className="p-5">
                      <p className="text-[10px] text-primary font-bold uppercase tracking-widest mb-1">{item.category}</p>
                      <h4 className="text-lg font-black headline tracking-tight mb-4 line-clamp-1 group-hover:text-primary transition-colors text-on-surface">{item.title}</h4>
