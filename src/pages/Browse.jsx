...
const Browse = () => {
  ...
  const CATEGORIES = ['All Categories', 'Fine Art', 'Tech & Innovation', 'Collectibles', 'Horology', 'Automobilia'];
  ...
  const filteredAuctions = auctions.filter((auction) => {
    ...
    // 3. Category Matching
    const matchesCategory = categoryFilter === 'All Categories' || auction.category === categoryFilter;
    ...
    <>
      <main className="max-w-[1600px] mx-auto px-8 pb-24 mt-12 flex flex-col lg:flex-row gap-12">
        ...
        <aside className="w-full lg:w-80 shrink-0">
          <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-3xl p-8 sticky top-28 shadow-sm">
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-outline-variant/20">
              <h2 className="text-2xl font-black headline tracking-tighter">Filters</h2>
              <button onClick={clearFilters} className="text-xs uppercase font-label font-bold text-on-surface-variant hover:text-primary transition-colors">
                Clear All
              </button>
            </div>
            ...
            <div className="space-y-8">
              {/* Search */}
              <div>
                <label className="block text-xs uppercase font-label font-bold text-outline mb-2">Search Catalog</label>
                <div className="relative w-full">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-outline text-[18px]">search</span>
                  <input 
                    type="text"
              ...
              {/* Status */}
              <div>
                <label className="block text-xs uppercase font-label font-bold text-outline mb-2">Auction Status</label>
              ...
              {/* Categories */}
              <div>
                 <label className="block text-xs uppercase font-label font-bold text-outline mb-2">Department</label>
                 <div className="space-y-2">
                   {CATEGORIES.map(cat => (
                     ...
                     >
                       {cat}
                       {categoryFilter === cat && <span className="material-symbols-outlined text-[16px]">check</span>}
                     </button>
                   ))}
                 </div>
              </div>
              ...
              {/* Price Range */}
              <div>
                 <label className="block text-xs uppercase font-label font-bold text-outline mb-2">Price Range (USD)</label>
        ...
        <section className="flex-1 w-full min-w-0 overflow-hidden">
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-outline-variant/20">
            <h1 className="text-4xl font-black headline tracking-tighter">Collections</h1>
            <div className="text-sm text-on-surface-variant font-label font-bold uppercase tracking-widest bg-surface-container-low px-4 py-1.5 rounded-full">
              {filteredAuctions.length} {filteredAuctions.length === 1 ? 'Lot' : 'Lots'}
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredAuctions.map((auction) => (
              <Link key={auction._id} to={`/auction/${auction._id}`} className="group bg-surface-container-lowest rounded-2xl overflow-hidden shadow-sm border border-outline-variant/20 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl block">
                <div className="relative h-64 overflow-hidden rounded-xl m-3 bg-surface-container shrink-0">
                  {auction.images && auction.images.length > 0 ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center text-outline bg-surface-container-low transition-transform duration-700 group-hover:scale-105">
                       ...
                       <span className="text-xs font-label">No preview visible</span>
                    </div>
                  )}
                  
                  <div className={`absolute top-3 left-3 bg-surface-container-lowest/90 backdrop-blur px-3 py-1.5 rounded-full flex items-center gap-2 ${auction.status !== 'active' ? 'opacity-70' : ''}`}>
                    <span className={`w-2 h-2 rounded-full ${auction.status === 'active' ? 'bg-secondary animate-pulse' : 'bg-outline'}`}></span>
                    <span className="text-[10px] font-bold uppercase tracking-widest font-label">{auction.status === 'active' ? 'Live' : 'Ended'}</span>
                  </div>
                </div>
                
                <div className="px-5 pb-5">
                  <div className="flex justify-between items-start mb-2 mt-1">
                    <p className="text-[10px] text-on-surface-variant font-label uppercase tracking-widest">{auction.category || 'Uncategorized'}</p>
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-outline-variant/10 flex justify-between items-end">
                    <div>
                      <p className="text-[10px] uppercase text-on-surface-variant font-label tracking-widest mb-0.5">Current Bid</p>
                      <p className="text-xl font-black headline text-primary">${auction.currentBid > 0 ? auction.currentBid : auction.startingBid}</p>
                    </div>
          ...
          {filteredAuctions.length === 0 && (
            <div className="py-32 text-center border-2 border-dashed border-outline-variant/30 rounded-3xl bg-surface-container-lowest">
              ...
              <p className="text-on-surface-variant font-body max-w-md mx-auto mt-2">Try adjusting your filters or search terms to uncover hidden gems in our catalog.</p>
              <button onClick={clearFilters} className="bg-on-surface text-surface-container-lowest px-6 py-2 rounded-full font-bold mt-6 hover:bg-primary transition-colors">
                Clear all filters
              </button>
            </div>
          )}
