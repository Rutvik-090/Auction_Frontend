...
const AuctionListing = () => {
  ...
  const filteredAuctions = auctions.filter((auction) => {
    ...
    // 3. Category Matching
    const matchesCategory = groupCategories.length === 0 || groupCategories.includes(auction.category);

    // 4. Price Matching
    const activePrice = auction.currentBid > 0 ? auction.currentBid : auction.startingBid;
  ...
  if (loading) return <div className="text-center py-40 text-on-surface-variant font-body animate-pulse">Loading curated gallery...</div>;
          ...
          <div className="px-6 py-4 flex flex-col gap-6">
            ...
            <div>
              <h3 className="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-4 font-headline flex justify-between">
                Search
                { (searchTerm || groupCategories.length > 0 || minPrice || maxPrice || filterStatus !== 'all') && 
                  <button onClick={clearFilters} className="text-primary hover:underline text-[10px]">Clear</button> 
                }
              </h3>
              <input 
                type="text"
            ...
            {/* Price Range */}
            <div>
              <h3 className="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-4 font-headline">Price Range</h3>
            ...
            <div>
              <h3 className="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-4 font-headline">Condition</h3>
              <div className="flex flex-wrap gap-2">
                <span className="px-3 py-1 bg-primary-fixed text-on-primary-fixed rounded-full text-xs font-semibold cursor-pointer">Mint</span>
                <span className="px-3 py-1 bg-surface-container-highest text-on-surface-variant rounded-full text-xs font-semibold cursor-pointer hover:bg-primary-fixed transition-colors">Excellent</span>
                <span className="px-3 py-1 bg-surface-container-highest text-on-surface-variant rounded-full text-xs font-semibold cursor-pointer hover:bg-primary-fixed transition-colors">Good</span>
              </div>
            </div>
        ...
        <main className="flex-1 w-full p-6 md:p-8 bg-surface">
          <section className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
              <div>
                <span className="text-primary font-bold text-sm tracking-widest uppercase">Live Curator Collection</span>
                <h1 className="text-4xl md:text-5xl font-black tracking-tight mt-2 text-on-surface headline">Discover Masterpieces</h1>
              </div>
              
              <div className="flex items-center gap-4">
                <div className="relative group">
                  <label className="text-[10px] absolute -top-2 left-3 bg-surface px-1 font-bold text-on-surface-variant uppercase tracking-tighter z-10">Sort By</label>
                  <select 
                    value={sortOption}
            ...
            {filteredAuctions.length === 0 ? (
              <div className="py-24 text-center border-2 border-dashed border-outline-variant/30 rounded-3xl bg-surface-container-lowest">
                <span className="material-symbols-outlined text-4xl text-outline-variant mb-4 bg-surface-container p-4 rounded-full">search_off</span>
                <p className="text-xl headline font-black text-on-surface tracking-tighter mt-4">No collections found.</p>
                <p className="text-on-surface-variant font-body max-w-md mx-auto mt-2">Try adjusting your filters or search terms.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                {filteredAuctions.map((auction) => {
                  ...
                  const activePrice = auction.currentBid > 0 ? auction.currentBid : auction.startingBid;
                  return (
                    <Link key={auction._id} to={`/auction/${auction._id}`} className="group bg-surface-container-lowest rounded-xl overflow-hidden shadow-[0_8px_32px_0_rgba(19,27,46,0.04)] hover:shadow-[0_12px_48px_0_rgba(19,27,46,0.08)] transition-all duration-300 block">
                      <div className="relative aspect-[4/3] overflow-hidden bg-surface-container-high rounded-t-xl">
                        ...
                        <div className={`absolute top-4 left-4 bg-secondary-container/90 backdrop-blur-md px-3 py-1.5 rounded-full flex items-center gap-2 ${!isActive && 'opacity-60 grayscale'}`}>
                          <span className={`w-2 h-2 rounded-full ${isActive ? 'bg-secondary animate-pulse' : 'bg-outline-variant'}`}></span>
                          <span className="text-[10px] font-black uppercase tracking-widest text-on-secondary-container">{isActive ? 'Live Bidding' : 'Ended'}</span>
                        </div>
                      </div>
                      
                      <div className="p-6">
                        <div className="flex justify-between items-start mb-2">
                          <h3 className="text-xl font-bold font-headline text-on-surface group-hover:text-primary transition-colors line-clamp-1">{auction.title}</h3>
                          <span className="material-symbols-outlined text-on-surface-variant cursor-pointer hover:text-error transition-colors mt-0.5 text-[20px]">favorite</span>
                        </div>
                        <p className="text-sm text-on-surface-variant mb-6 line-clamp-1">{auction.category || 'Uncategorized Asset'}</p>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                          ...
                          <div className={`p-3 rounded-xl border ${isActive ? 'bg-tertiary-container/10 border-tertiary/10' : 'bg-surface-container border-outline/10'}`}>
                            ...
                            <span className={`text-lg font-black font-headline ${isActive ? 'text-tertiary' : 'text-outline'}`}>
                               {isActive ? 'Active' : 'Closed'}
                            </span>
                          </div>
                        </div>
                        
                        <button className={`w-full py-4 rounded-full font-bold shadow-lg active:scale-[0.98] transition-all ${isActive ? 'bg-gradient-to-r from-primary to-primary-container text-white shadow-primary/20 hover:shadow-primary/40' : 'bg-surface-container-high text-on-surface-variant cursor-not-allowed'}`}>
            ...
            {filteredAuctions.length > 0 && (
              <div className="mt-20 flex flex-col items-center gap-6 pb-12">
                <p className="text-on-surface-variant text-sm font-medium">Displaying {filteredAuctions.length} exclusive lots</p>
      ...
      <footer className="bg-surface-container-low border-t border-transparent pt-16 pb-8 px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          <div className="col-span-1 md:col-span-2">
            <span className="text-2xl font-black text-primary tracking-tighter block mb-6 headline">The Auction Curator</span>
            <p className="text-on-surface-variant max-w-sm mb-8 leading-relaxed">The premier destination for the world's most exceptional assets. Our curated platform connects discerning collectors with masterpieces of history, art, and engineering.</p>
          ...
          <div>
            <h4 className="font-bold font-headline text-on-surface mb-6 uppercase tracking-widest text-xs">Navigation</h4>
            <ul className="space-y-4">
              <li><a className="text-sm text-on-surface-variant hover:text-primary transition-colors" href="#">Active Auctions</a></li>
              <li><a className="text-sm text-on-surface-variant hover:text-primary transition-colors" href="#">Private Treaty Sales</a></li>
              <li><a className="text-sm text-on-surface-variant hover:text-primary transition-colors" href="#">Collector's Journal</a></li>
              <li><a className="text-sm text-on-surface-variant hover:text-primary transition-colors" href="#">Sell with Us</a></li>
            </ul>
          </div>
          <div>
            <h4 className="font-bold font-headline text-on-surface mb-6 uppercase tracking-widest text-xs">Curator Trust</h4>
            <ul className="space-y-4">
              <li><a className="text-sm text-on-surface-variant hover:text-primary transition-colors" href="#">Authenticity Guarantee</a></li>
              <li><a className="text-sm text-on-surface-variant hover:text-primary transition-colors" href="#">Escrow Protection</a></li>
              <li><a className="text-sm text-on-surface-variant hover:text-primary transition-colors" href="#">Global Logistics</a></li>
